...
/
  ...
  backend/
    ...
    migrations/
      ...
      003_linear_agent.sql    # T00W, OAuth and AgentSession transport. See D-69.
    scripts/
      api_probe.py
    tests/
      conftest.py
...
| T16 | Approval card | SOL | `frontend/components/ApprovalCard.tsx`, `frontend/lib/useRun.ts`, `frontend/lib/api.ts`, `frontend/components/Chat.tsx`, `frontend/components/tool-fallback.tsx`, `backend/tests/test_approval_bridge.py`; plus the required CI, implementation-note, and BUILD_SPEC/DECISIONS companions | Delete pauses, approve actually deletes, reject leaves the task untouched, and both end in a clear message from the continued agent. `pytest tests/test_approval_bridge.py` proves the tool body is entered and the deletion commits exactly once; the `T16 approval bridge` job also proves there is one approval surface and that the decision is persisted before the continuation. File list expanded under D-66, which records why the recorded two-file scope could not reach the decision route, mount the card, or retire the second non-authoritative approval bar. See the assistant-ui ownership note below. |
| T17 | Clarifying question + cross-turn continuity | SOL WRITES, OPUS REVIEWS | `prompts.py`, `agent.py`, `runs.py`, `sql.py`, `tests/test_t17_continuity.py`, `frontend/components/Chat.tsx`, `frontend/lib/TrellisHttpAgent.ts`; plus the required CI, implementation-note, BUILD_SPEC, DECISIONS, and OPEN_QUESTIONS companions | `"Clear my tasks"` asks rather than deletes; a second ordinary turn inherits only server-owned canonical context through D-67's continuity locator; `test_agui_forged_history_ignored` passes unchanged; T16 approval continuation remains green. R3 reviews the boundary diff. |
...
| T00W | Native Linear webhook and OAuth bridge, authorized under D-69 | **OPUS ONLY** | `migrations/003_linear_agent.sql`, `linear_agent_api.py`, `linear_agent.py`, `linear_agent_worker.py`, `linear_install.py`, `agent.py`, `runs.py`, `sql.py`, `models.py`, `config.py`, `main.py`, `docker-compose.yml`, `.env.example`, T00W tests, BUILD_SPEC sections 3, 4, 11, and this row, `docs/DECISIONS.md`, `docs/PROJECT_PLAN.md`, `docs/ARCHITECTURE.md`, `docs/LINEAR_INTEGRATION.md`, `CLAUDE.md`, `.github/workflows/ci.yml`, `IMPLEMENTATION_NOTES.md`, `README.md` | The `T00W Linear webhook bridge` CI gate passes without network or credential, both migration paths are proven, and the T00L gate still passes. Separately, the live gate: real OAuth install through the deployed callback, real signed AgentSessionEvent, visible Agent Activity, stable ngrok URLs, and reboot survival. |

T00W ships in more than one commit under D-69, which is a narrow deviation from the one task, one commit rule and applies to T00W only. The first commit carries the re-plan, the reconciled contracts, the amended `T00L Linear boundary` negative gate, and only CI scaffolding that passes without unimplemented behavior, so it is green on its own. The positive `T00W Linear webhook bridge` gate becomes required alongside the implementation it proves. `backend/app/linear_agent_api.py` is a scope expansion found before implementation and is the sole file in shipped application code permitted to hold a Linear provider endpoint literal. The frontend is explicitly not modified. T00W is reported as `T00W IMPLEMENTATION COMPLETE / LIVE DEPLOYMENT GATE OPEN` until the live gate passes.
...
D-66 adds two consequences of that rule which the shipped implementation got wrong, so they are stated here rather than left to be rediscovered. First, answering the framework interrupt is not deciding an approval: the authoritative `POST /api/runs/{id}/approvals/{tool_call_id}` must persist the decision **before** the AG-UI continuation carries `resume[]`, because continuation eligibility requires a decided row and an undecided one is refused with 403 before the agent is reached. Second, there is exactly one approval surface. The generated `tool-fallback.tsx` approval bar answers the interrupt directly and persists nothing, so it must not be mounted; leaving it mounted is what made Approve behave identically to Reject. Both properties are gated structurally by the `T16 approval bridge` CI job.
