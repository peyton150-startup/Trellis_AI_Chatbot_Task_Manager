...
8. **`RUN_FINISHED` comes after the run is durably `completed`, and a failure
   ...
   **Each invocation is bound to the run the server issued for it.** The first
   ...
   them. `input.runId` is the AG-UI invocation identifier, generated by the
   client per invocation unless a caller explicitly supplies one, which the
   protocol permits. It is used only to look up which application run this
   invocation belongs to; it is never sent to the server as authority and never
   reaches `fetchRun`. Trellis does not supply or override it: `TrellisHttpAgent`
   spreads the input and replaces `forwardedProps` alone, and a gate asserts
   that.
